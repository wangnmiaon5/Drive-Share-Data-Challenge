...
corr_df = train[['pickup_datetime','pickup_weekday', 'pickup_dayofyear', 
       'pickup_hour', 'pickup_minute', 'night_trip', 'rush_hour',
...
ax.set(xlabel = "Day of Week", ylabel = "Count of Lyft Rides")
plt.show()
...
plt.figure(figsize=(10,4))
ax = sns.barplot(x = day, y = count)
ax.set(xlabel = "Hour in Day", ylabel = "Count of Lyft Rides")
plt.show()
...
# revise colla a little bit
colla1 = pd.read_csv('colla1.csv')

## very high demand data
colla1 = colla1[(colla1['row_id_count']>1930)& (colla1['rush_hour_']==True)]

ax = sns.lmplot(x="pace_median", y="row_id_count", hue='rush_hour_', data=colla1, lowess=True)
ax.set(xlabel='Trip Pace (Median): Duration / Distance', ylabel='Demand (Total Ride Counts)')
plt.savefig('/Users/miaowang/Box Sync/third paper/proposal/lyft_demand_pace.png')
plt.savefig('/Users/miaowang/Box Sync/conference/lyft_demand_pace.png')


ax = sns.lmplot(x="trip_distance_median", y="row_id_count", hue='rush_hour_', data=colla1, lowess=True)
...
plt.savefig('/Users/miaowang/Box Sync/conference/lyft_demand_distance.png')


ax = sns.lmplot(x="duration_median", y="trip_distance_count",  hue='pickup_hour_', data=colla1, lowess=True)
...
ax = sns.boxplot(x="pickup_hour_", y="row_id_count", data=colla1)
...
ax = sns.boxplot(x="pickup_hour_", y="pace_median", data=colla1)
ax.set(xlabel='Pick Up Hour', ylabel='Pace (Median)')
plt.savefig('/Users/miaowang/Box Sync/conference/lyft_pace.png')
...
data = [[x[0], x[1], 1] for x in np.array(colla1[['center_latitude_mean',
        'center_longitude_mean']])]
...
#########################################################
RANDOM_STATE = 42
train_x, val_x, train_y, val_y = train_test_split(train_attr, y, test_size=0.2, random_state = RANDOM_STATE)
...
##########################################
##### plot random forest importance plot
# https://stackoverflow.com/questions/44101458/random-forest-feature-importance-chart-using-python
##########################################
rnd_clf = RandomForestRegressor(n_estimators=100, max_depth=10, max_features=0.5, random_state=42)
rnd_clf = RandomForestRegressor(random_state=42)
rnd_clf.fit(train_attr, y)
for name, importance in zip(new_df.dtypes.index, rnd_clf.feature_importances_):
...
y_true, y_pred = y, rnd_clf.predict(train_attr)
mae(y_true, y_pred)

clf = rnd_clf.fit(train_x, train_y)
train_mae = mae(train_y, model.predict(train_x))
    val_mae = mae(val_y, model.predict(val_x))
    print('With model: {}\nTrain mae: {}\nVal. MAE: {}'.format(model, train_mae, val_mae))
